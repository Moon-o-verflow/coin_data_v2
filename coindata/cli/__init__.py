...
명령: `init`(UF-1), `sync`(UF-2), `summary`(UF-3, 2단계), `status`(UF-4).
...
def build_parser() -> argparse.ArgumentParser:
    ...
    commands.add_parser("summary", help="요약을 만든다 (UF-3, 2단계에서 구현)")
...
def main(argv: Sequence[str] | None = None, runtime: Runtime | None = None) -> int:
    ...
    if args.command == "summary":
        print("summary 명령은 2단계(compute·report)에서 구현한다.", file=sys.stderr)
        return EXIT_CANNOT_RUN
